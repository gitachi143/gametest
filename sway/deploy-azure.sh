#!/usr/bin/env bash
#
# Deploy SWAY to Azure Container Apps.
#
#   ./sway/deploy-azure.sh
#
# ACR builds the image server-side, so no local Docker is needed - it is the
# same shape as the Cloud Build path in deploy.sh.
#
# The build context is sway/, so the other app's source is never uploaded.
#
...
#   APP               container app name        (default: sway)
...
# shared with Nexus Arcade - one Container Apps environment costs less than two, and
...
# --- 7. provider wiring ---------------------------------------------------
# --max-replicas 1 keeps every player on one SQLite file, so the leaderboard and
# unlocked levels are shared. Raise it only after swapping app/store.py for a
# networked database.
ENV_VARS=("LLM_PROVIDER=${LLM_PROVIDER}" "LLM_MODEL=${LLM_MODEL}" "DB_PATH=/tmp/sway.db"
          "LLM_CONCURRENCY=${LLM_CONCURRENCY:-3}")
SECRETS=()
